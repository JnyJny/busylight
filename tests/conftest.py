...
def generic_usblight():
    ...
    with mock.patch("hid.device"):
        light = USBLight(0xFFFF, 0xFFFF, 0xDEAD_BEEF)
...
def blinkstick():
    ...
    with mock.patch("hid.device"):
        light = BlinkStick(vendor_id, 0xFFFF)
    return light
...
def blynclight():
    ...
    with mock.patch("hid.device"):
        light = Blynclight(vendor_id, 0xFFFF)
    return light
...
def busylight():
    ...
    with mock.patch("hid.device"):
        light = BusyLight(vendor_id, 0xFFFF)
    return light
